...
/* mode_ring: 2 phase 切换 coarse 矩形框 vs 细圆环 per-bit 尝试.
 * phase 0: 3×8 cell 矩形外框 (一定能画)
 * phase 1: 48×8 per-bit 圆环 (尝试单 LED 分辨率) */
static void mode_ring(void)
{
    static int init = 0;
    if (!init) {
    ...
    static u32 frame = 0;
    frame++;
    u32 phase = (frame / 120) % 2;   /* 2s per phase */

    /* Precompute per-chip masks for both phases */
    u16 chip_masks[8][3];   /* [chip][region 0=right, 1=mid, 2=left] */
    if (phase == 0) {
        /* COARSE rectangle frame: top/bot 全亮 + left/right column 全亮 */
        for (int chip = 0; chip < 8; chip++) {
            u16 left = 0, mid = 0, right = 0;
            if (chip == 0 || chip == 7) {  /* top/bot row → all cells on */
                left = mid = right = 0xFFFF;
            } else {  /* middle rows → only left+right cells (frame edges) */
                left = right = 0xFFFF;
                mid = 0;
            }
            chip_masks[chip][0] = right;
            chip_masks[chip][1] = mid;
            chip_masks[chip][2] = left;
        }
    } else {
        /* FINE per-bit circle: 48 col × 8 row 假设 bit linear = col offset */
        for (int chip = 0; chip < 8; chip++) {
            for (int region = 0; region < 3; region++) {
                u16 mask = 0;
                for (int bit = 0; bit < 16; bit++) {
                    /* col 0..47 = region 0 [0-15], region 1 [16-31], region 2 [32-47] */
                    int col = region * 16 + bit;
                    int row = chip;
                    int dx = col - 24;
                    int dy = (row - 4) * 6;   /* scale 8 row to ~48 (aspect) */
                    int d2 = dx*dx + dy*dy;
                    /* ring r^2 ∈ [225, 400] = r in [15, 20] */
                    if (d2 >= 225 && d2 <= 400) {
                        mask |= (u16)(1u << bit);
                    }
                }
                chip_masks[chip][region] = mask;
            }
        }
    }
    ...
    for (int row_iter = 0; row_iter < 384; row_iter++) {
        ...
        for (u32 latch = 0; latch < CHIPS_PER_CHAIN; latch++) {
            ...
            u16 vals[9] = {0};
            if (chip < 8) {
                u16 right = chip_masks[chip][0];
                u16 mid   = chip_masks[chip][1];
                u16 left  = chip_masks[chip][2];
                /* white ring = R+G+B all chains */
...
/* mode_verify: 6 phase × 60 帧, 系统测试 24 cell 独立性 + 位置准确度.
 * 期望: 每 phase panel 只亮指定 cell, 别处全黑 → cell control 真独立. */
static void mode_verify(void)
{
    static int init = 0;
    if (!init) {
    ...
    static u32 frame = 0;
    frame++;
    u32 phase = (frame / 60) % 6;

    /* lit_mask[chip(0..7)][region(0=left,1=mid,2=right)] = bit (0=off, 1=on white) */
    u8 lit[8][3] = {0};
    switch (phase) {
        case 0: lit[0][0] = 1; break;                  /* top-left only */
        case 1: lit[0][2] = 1; break;                  /* top-right only */
        case 2: lit[7][0] = 1; break;                  /* bot-left only */
        case 3: lit[7][2] = 1; break;                  /* bot-right only */
        case 4: lit[0][0] = 1; lit[7][2] = 1; break;   /* diagonal TL+BR */
        case 5: /* cross: mid row 全亮 + mid col 全亮 */
            for (int r = 0; r < 8; r++) lit[r][1] = 1;
            for (int c = 0; c < 3; c++) lit[3][c] = lit[4][c] = 1;
            break;
    }
    ...
    for (int row_iter = 0; row_iter < 384; row_iter++) {
        ...
        for (u32 latch = 0; latch < CHIPS_PER_CHAIN; latch++) {
            ...
            u16 vals[9] = {0};
            if (chip < 8) {
                /* left=chain6-8, mid=chain3-5, right=chain0-2; 全白 = R+G+B */
                u16 left  = lit[chip][0] ? 0xFFFF : 0;
                u16 mid   = lit[chip][1] ? 0xFFFF : 0;
                u16 right = lit[chip][2] ? 0xFFFF : 0;
                vals[0] = vals[1] = vals[2] = right;
                vals[3] = vals[4] = vals[5] = mid;
                vals[6] = vals[7] = vals[8] = left;
            }
