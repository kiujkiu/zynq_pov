...
### 7. 自制 chip symbol — sub-symbol 名不能带 namespace
见 `feedback_kicad_subsym_namespace.md`. `define_chip()` 已经处理.

### 8. 文字布局 (Ref/Val/label) 全自动避让
- `_ref_val_positions(s, pin_worlds)`: 2-pin 横/竖 passive 用 side placement, 多 pin chip 用 above/below bbox (offset 7.62mm 留给 stub label)
- `_emit_power_sym` hide value text: GND 家族 + PWR_FLAG (避免跟 attached label 撞)
- `_emit_symbol` hide value text: 同上对 `power:` 前缀
- `LABEL_ROT = {0: 180, 180: 0, 90: 0, 270: 0}` — 所有 global label 文字横向阅读
- `_emit_glabel` 自动 justify: rotation 180/270 → right, 否则 left (文字朝外, 不压 chip body)
- `find_text_overlaps()` 检测器: 算每个 text bbox 找重叠, gen 完跑一次确认 0 重叠
- module_box(name, start, end): 矩形框 + 加粗标题 (justify left bottom), 视觉分块
