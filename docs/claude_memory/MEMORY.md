...
- [ICND2260 规格与 POV 可行性 (2026-08-04)](reference_icnd2260_spec.md) — 行列合一 120通道+48行管; mLVDS 3对=1.02Gbps/颗; **帧频=GCLK/(灰度等级×扫描数)**; 线上位宽固定16bit; 色深天花板 10-bit; 🔴 瓶颈变成差分对数(96对), 双面30Hz需146对 ⇒ 首次出现换 FPGA 的技术理由 ; 🔴 **08-27 三处更正**: 帧频公式是**上界**漏了消影税 ⇒ 605片/9075Hz 下 10-bit 余量只有 **2.5%** 不是 34.5%(且 DCLK 必须精确 160MHz, 顶到 166.7 反而不过); "真16-bit物理不可能"说法不准 —— S=1 时规格内成立, 是**工程出局**(2304颗+576对+5-7kW); **线上位宽不可选 ⇒ 降色深省不了一根差分对**, 省的是 DDR/WiFi 字节数
...
- [🎯 终极目标的选型判决 (605片/30Hz/9075Hz)](project_pov3d_final_target_chip_verdict.md) — **两块在手的板都不成立, 但原因不是芯片选错, 是板子和架构**; 🔴 瓶颈在**开发板连接器只引出 96 根**(反推片/圈上限只有 **210**) ⇒ 换 FPGA 一根线都不多给; 🔴 **"外扩 CPLD"不成立** —— 比特率守恒, 2260 线上无压缩空间, **扇出只复制比特不减比特率**, 省脚只能靠 TDM 而 TDM 要 ISERDES(那是 CPLD 与 FPGA 的分界线); 唯一例外是**转子侧串化桥**(靠 SERDES, CPLD 恰恰没有); **DR1 不够但与 7020 几乎逐条打平**(差分对/板级引出/LUT/**DDR 全打平**), DR1 只多 5 条短板(单端 300Mbps<需要的 333 / LVDS 直流参数全 TBD / HP 2口 / CMA 96MB / 无 SERDES); 🎯 **最该做的一件事是问供应商 `VHEAD[23:20]` 有没有 8-bit 线上格式** —— 若有则预算减半、96 对够用、外置驱动器都不需要
...
- [🔴 DR1 只有 2 个 HP, Zynq 现役用满 4 个](project_dr1_hp_port_shortage.md) — 移植头号架构问题; 面板占 1 口 ⇒ 三个 lz4 引擎的读+写挤剩下 1 个; '每引擎独占一口'是设计理由不是随手分(前车之鉴 NUM_SI=8 挤一口把 HDMI 搞成噪点); **TD 没有 SmartConnect 对等 IP, N:1 汇聚要手写**; 带宽账 253MB/s 但 **DR1 HP 实际带宽零实测** ⇒ ✅ **已定案**, 见下条
- [🎯 DR1 HP 拓扑定案 — 方案A](project_dr1_hp_topology_decision.md) — 面板独占 HP0@50M + **2 个 lz4 引擎 @100MHz** 经 2:1 仲裁打 HP1(退路方案C: 3 引擎@50M, **只差两个参数**); 🔴 关键发现 **两个 HP 口在 DDRC 前就合流成单条 `HP_BUS`** ⇒ 口分开买不到 Zynq 那种 DDRC 级隔离, 只买到 **AXI 通道级隔离**(而那才是当年 HDMI 变噪点的真病根: 头阻塞); 🔴 **HP1 在当前 PS IP 里根本没生成**(线全接常量且是 32 位), 要用 TD **重新生成 PS IP**, 光改 wrapper 不够; 交付 `rtl/axi_hp_arb.v`(一主一固定 ID 免重排序 / W 走 AW 顺序 FIFO 因为 **HP 口没有 WID** / N=1 退化成纯位宽适配器), 仿真 7 档+N=1..4 全过 + **变异 7 个 bug 全被抓**(bug3 恒 ready 下是 PASS); 🎯 写桥不用新写(`lz4_engine_axi` 已攒成 ≤16 拍 INCR); 🔴 **没过 TD 综合, 资源/Fmax 零实测** ⇒ ✅ **08-27 落地**: 仲裁器过 TD(N=2 @100MHz 余量 43%, 0.35% slice) + **HP1/p2f_clk1 已接出**(`tools/ps_enable_hp1.py`, 不用重新生成 PS IP, `GEN_PL_HP1` 时钟域实证); 🔴 但那只是 PL 侧一半 —— PS 侧还要写 `0xF8801078[1]`/`0xF8800084[1]`/`0xF880103C[13:8]`(+ 疑似 `0xF8806330[7:4]`, **它不在厂商寄存器表里, BOOT.bin 永远不写**), 不写就是"综合全过、上板静默不应答"
