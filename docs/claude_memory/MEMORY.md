...
- [🎯 鹿小班 × ICND2260 首次点亮工程 (2026-08-19)](project_lxb_icnd2260_bringup.md) — 转接板 zynq-lxb_2260_V0.1 网表(信号用 site 名标注); **两块 2260 板只有鹿小班这块画完了**, 且 SCH 的 PDF 是旧版会骗人; 定 TTL 3 通道(3.3V bank 出不了 LVDS); 电源 EN 归 FPGA 管 = 唯一复位手段; 🔴 **08-19 改走 mini-LVDS**(分支 `feature/icnd2260-lvds`): 鹿小班 **BANK 33 是唯一挂 VCCIO_ADJ 的 bank**, 换 U6 的 R13 33K→22.6K 就是 2.5V ⇒ LVDS_25 可用; LVDS 与 TTL 有四处实质差别(2.5V/时钟90°移相/第一通道是B不是R/寄存器两个使能位); 代码在 `zynq_pov/icnd2260/`, 🎯 **08-19 首次上板: FPGA 侧全通**(VIO 实测 fps 2648@41.7M / 1324@20.8M, 与手推模型差 0.06%), 但**屏侧零回包**(ack frame/err 都是 0 ⇒ 指向物理层不是协议层); 🔴 **未解决: Altium 转接板与我们 KiCad 面板的连接器脚位完全不兼容**(3.8V 会灌进面板 GND 和芯片 LVDS 输入), 核对前别插电
