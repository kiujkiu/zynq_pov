...
## ✅ 2026-08-04 晚 追加: 打到极限 = **~125 Mbps**, 200M 做不到

| 方法 | 结果 |
|---|---|
| WSL 侧 TCP 500 MB 单流 | **115.9 Mbps** (尾段 130) |
| **Windows 侧 TCP 500/300 MB** (绕开 WSL2 NAT) | **110.6 / 124.1 Mbps** |
| 1/2/4/8/16 并发流聚合 | 128/121/86/101/107 —— **多流不涨** |
| UDP 灌 296 Mbps, 到达内核的量 | ~94 Mbps (接收端 Python 还在丢, 是**下限**) |
| 板内回环 | **1123 Mbps** |

### 已排除 (都有硬证据)
| 嫌疑 | 证据 |
|---|---|
| **WSL2 NAT** | Windows 直连 110.6 ≈ WSL 115.9 ⇒ 不是 |
| 板子 CPU/协议栈 | 回环 1123 Mbps; 传输时两核各 **50% idle** |
| 空口质量 | signal −41~−44 dBm, ack −37 dBm, **tx retries 0 / tx failed 0** |
| 驱动丢包 | `rx_dropped=0 rx_errors=0` |
| TCP 窗口/单流 | 多流聚合不涨 (86-128) |

### 🔴 核心矛盾: 空口协商 432-540, TCP 只有 124
```
iw station dump 传输中采样: rx bitrate 324-540 MBit/s @80MHz, channel 149
TCP 实测 124 Mbps  ⇒ 效率仅 25-28% (正常 WiFi 应为 PHY 的 50-65%)
```
**理论上还有一倍空间, 但这段差距未归因。** 两个候选都没排除:
1. mt76 的 USB 批量传输开销
2. 办公 AP 空口争用 (SSID `undef`; ⚠ `iw survey dump` 只报 2.4G 且全 0, 驱动没实现, 测不到信道忙碌率)

⚠ `txpower` 报 **3.00 dBm** 且 `iw set txpower` 改不动 —— mt7921u 大概率**没实现**这两个接口,
不是真功率低 (ack 在 AP 侧 −37 dBm 很强)。别被这个数误导。

### 要往 200 走的三条路 (按性价比)
1. **同口插 U 盘测裸吞吐** ⭐ 决定性且几乎免费, **需要物理插一根 U 盘**
   - U 盘 250+ Mbps ⇒ 是 mt76 的锅, 换 dongle 芯片有戏
   - U 盘也 ~125 ⇒ ci_hdrc 主控上限, **USB 这条路到顶**
2. 接个专用 5G 路由器 ⇒ 排除办公网争用
3. 换 dongle 芯片 (要重编内核, 环境已丢) —— 只有 1/2 指向 mt76 才值得

### 但先想清楚要不要 200
| 需求 | 需要 | 125 够吗 |
|---|---|---|
| 现 PVS1 720片双面 @30fps | 63 Mbps | ✅ 2× 余量 |
| ICND2260 体素表 @**10Hz** 内容 | 25-42 Mbps | ✅ 3-5× 余量 |
| ICND2260 体素表 @30Hz 内容 | 108-216 Mbps | ⚠ 贴边到不够 |

⇒ **只有"满屏精细模型 + 30Hz 内容更新"才需要 200M**, 而
[[project_pov3d_icnd2260_datapath]] 已确认 **内容帧率 ≠ 体帧率** (屏 30Hz 转但姿态 10Hz 更新)。
⇒ **不建议追 200。** 现在真正卡帧率的是 **ARM 解码 78-88 ms/帧 = 11-12 fps**,
链路有 2 倍余量闲着。
