...
> ⇒ "口分开"在 DR1 上买不到 DDRC 级隔离~~
> 🔴 **2026-08-27: 那条"合流"本身又被厂商推翻 —— 两个 HP 口不合流。**
> 两路**各有独立 HP Bus + 各自独立的读缓存与写缓存 + 各自独立的 OCM 出口(M0/M1)**, 且公共缓存内部**读/写/OT 三者相互独立** ⇒ 🎯 **lz4 的写洪水吃不掉面板的读缓存条目**。
> ⇒ 口分开买到的比"只买到通道级隔离"**多**(通道级隔离 + 独立 Bus + 独立读/写缓存);
> "头阻塞才是当年 HDMI 变噪点的真病根"这半仍成立。
...
| 面板 `ddr_slice_fetch256` | HP0 独占, **纯读** ~~110.8~~ → **约 160 MB/s**, 32-bit | 占 1 个 |
...
| DDRC 入口有 3 个 {HP0}/{HP1,HP2}/{HP3} ⇒ 写流量能 2:2 摊开 (AMD 文档: "HP1 and HP2 shares one DDR input port") | ~~❓ 没查过~~ ✅ **08-27 查实, 比 Zynq 更好**: 两个 HP **不合流**, 各自独立 HP Bus + 独立读/写缓存 + 独立 OCM 出口(M0/M1); 公共缓存内**读/写/OT 三者独立** ⇒ **写洪水吃不掉面板的读缓存条目**。⚠ 但 **M2/M3 按地址还是按 master 未决**(UG1214`:7275-7280` 按 master vs 厂商答复按地址, 冲突) |
| DDRC 拥塞时**读优先于写** ⇒ 面板是被偏袒的一方 | ~~❓ 没查过~~ 🔴 **08-27 厂商确定否认**: "不是简单固定读优先或写优先", 并点名读写方向切换/写队列批量排空/Bank-Row 冲突**仍可能造成读延迟尖峰** ⇒ **这半靠山在 DR1 上没了**; 且厂商明确**没有最小带宽/最大延迟保证**, 只有超时防饥饿(防饿死≠保时限) |

## 带宽账【推算】

~~三引擎 @50 MHz × 0.95 B/clk ≈ **142 MB/s 写** + 面板 **110.8 MB/s 读** ≈ **253 MB/s**~~
🔴 **08-27 重算**: 面板取片 110.8 是 **1-bit / 360 槽 / 780 RPM** 的旧账。
现役口径(3-bit 三位平面 × 双面 × **142 槽/面** × **969 RPM**):
`142 × 2 × 3 × 11664 B = 9.94 MB/圈 ÷ 61.9 ms ≈ **160 MB/s**`
⇒ 三引擎 **142.5 写** + 面板 **160 读** + 压缩流 6-7 读 ≈ **约 300 MB/s**, 全压在 2 个 64-bit 口上。
🔴 **而面板那条通路的硬顶是上游 32 位 @50 MHz = 200 MB/s**(仿真实测 176-185) ——
桥把一个 64 位拍拆成两个 32 位拍还给上游, **每 2 拍才收一个 64 位拍**
(`rtl/axi4_to_axi3_hp.v:118-120`) ⇒ **余量约 1.15×, 不是 `pov_dual_top.v:713` 注释写的 1.7×**。

~~🔴 **DR1 的 HP 实际可用带宽零实测** —— 这条论证整个悬在一个没有的数上。~~
🔴 **08-27 收口**: 厂商值 **1600 MB/s**(DS1200 **V1.0** 表 3-36; 我们看的表 3-34 是 0.4 草稿),
且 **XMON 自带带宽计数器(0xF8440000)可在板上实测** ⇒ 不再悬在一个没有的数上。
