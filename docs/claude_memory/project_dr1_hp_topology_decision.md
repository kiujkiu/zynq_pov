...
description: DR1 HP 拓扑定案(方案A) — 面板独占 HP0@50M + 2 个 lz4 引擎 @100MHz 经 2:1 仲裁打 HP1; 关键发现是两个 HP 口在 DDRC 前就合流成单条 HP_BUS, 且 HP1 在当前 PS IP 里根本没生成
metadata:
  type: project
---
...
⇒ 必须用 TD **重新生成 PS IP**(`GuiFinal.xml` 的 `HP1_Interface=1` + `HP1_DATA_WIDTH=64`),
**光改 wrapper 不够**。
⇒ "DR1 有 2 个 HP 口"在**芯片层面**成立, 在**当前工程层面只有 1 个**。这直接影响排期。
...
5. 🔴 **`axi_hp_arb` 没过 TD 综合**, 资源和 Fmax 一个数都没有(工具链当时被 A 线占着)。
   模块里用了 `$clog2`, TD 若不支持要换手写常量。
