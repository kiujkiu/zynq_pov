...
| 分配 | HP0 = 面板 `ddr_slice_fetch256` **纯读** 110.8 MB/s (32-bit)<br>HP1/HP2/HP3 = 三个 lz4 引擎**各独占一口** | 面板占 1 个 ⇒ **三个引擎的读+写全挤进剩下那 1 个** |
...
**带宽账**【推算】: 三引擎 @50 MHz × 0.95 B/clk ≈ 142 MB/s **写** + 面板 110.8 MB/s **读**
≈ 253 MB/s, 全压在 2 个 64-bit 口上。
...
| DDRC 入口有 3 个: {HP0}/{HP1,HP2}/{HP3} ⇒ 引擎写流量能 2:2 摊开 | ❓ DR1 的 DDRC 入口拓扑**没查过**, 厂商文档里未见对应说明 |
| DDRC 拥塞时**读优先于写** ⇒ 面板是被偏袒的一方 | ❓ 安路 DDRC 的仲裁策略**没查过** |
...
**v2 新增:**
- **DR1 的 DDRC 入口拓扑与读写仲裁策略** —— Zynq 那边"读优先于写 ⇒ 面板被偏袒"是有
  AMD 文档背书并被 08-25 实测印证的; 安路侧**两者都没查过**。这是 §3 能不能成立的关键。
