...
| ④ 真差分电气 | 🔴**2026-09-04 已更正, 见文末**: Bank1/3 锁 3.3V 属实但**不构成障碍**; 真正的隐患只有 **VOCM 0.6–1.4V 超出屏侧 VIC 1.1–1.3V** | 靠外部偏置或上板量 |
| ⑤ IOL 内 pclk↔sclk 相位 | ⚠ TD 不检查, F 接法 pclk 有 1.9 ns 插入延迟 | 只能上板量 |

⇒ **EG4S20 "技术上可行, 但三条轴同时零余量"**(时序 2% / 共模无保证 / 相位靠上板)。
🔴 **2026-09-04 更正: 降到 700 Mbps(fabric 175MHz)后时序那条变 16.7%, 只剩两条。见文末。**
...
# 🔴 2026-09-04 更正: ④ 那条推理链是错的

## 错在哪

原文写 "Bank1/3 锁 3.3V ⇒ **RX 必须放 Bank0/2** ⇒ 33 対真対驱屏共模无保证"。
**中间那一步是我自己推的, 手册不支持。** 已回原文逐条核:

【DS301 原文】BG256 与 CG324 两张引脚表下**各有一份同样的注**, 一字不差:
> 因为内部 BANK1 与 BANK3VCCIO 与 SDR SDRAM 电源相连, 所以 VCCO1、VCCO3 只能接 3.3V 电源,
> 且 **VCCIO3、VCCAUX、VCCIO1 内部相互短路**, 使用时需要注意。

(比原记录更严: 不只短接 SDRAM, **还短接 VCCAUX** 这条 3.3V 硬轨, 没有绕法。)

## 但锁的正好是不需要 2.5V 的那两个 bank

【实数·CG324 引脚表数 P 脚】 + 【DS300 表 2-7-1: 左/右 = True LVDS 无需外部电阻, 上/下 = Emulated 需 3R】

| Bank | 前缀 | 対数 | 类型 | VCCIO |
|---|---|---|---|---|
| Bank1 | `L` | 20 | **真** | 锁 3.3V |
| Bank3 | `R` | 30 | **真** | 锁 3.3V |
| Bank0 | `BE` | 29 | 仿真 | **可 2.5V** |
| Bank2 | `TE` | 25 | 仿真 | **可 2.5V** |

合计 50 真 + 54 仿真 = 104, 与 DS301 表 1-2 `215/(50+54)` 精确吻合。

**唯一强制要 2.5V 的是仿真対**(输出标准表只有 `LVDS25E` @2.5V/166MHz, **没有 LVDS33E**),
而它们全在没被锁的 Bank0/2。真対全在被锁的 Bank1/3, 但真対不需要 2.5V:

- 【DS300 表 3-2-5】`LVDS33` 输入输出都是 **400 MHz / 800 Mbps**, 与 `LVDS25` 相同
- 【DS300 表 3-1-15】**VOCM 在 VCCIO=2.5V 与 3.3V 下是同一个窗口 0.6~1.4V**, VOD 也不分电压

⇒ **真対走 LVDS33 驱屏, 电气上与 LVDS25 无差别。RX 放 Bank1/3 也完全没问题**
(LVDS33 输入 VICM 0.6~3.15V / 400 MHz)。

## 真正还站着的隐患: 共模, 且与 bank 无关

【ICND2260 DS V1.7】共模输入电压 `VIC` = **1.1 / 1.2 / 1.3 V**(min/typ/max)
【EG4 DS300 表 3-1-15】`VOCM` = **0.6 ~ 1.4 V**

⇒ **EG4 的规格窗口两端都超出屏侧的接受窗口**, 而且 2.5V/3.3V 下完全一样, **换电压救不了**。
之前上板实测 1.294V 能收, 但那是单颗样品的实测值, 不是规格保证。

## 五项计分(更正后)

| # | 项 | 状态 |
|---|---|---|
| ① 仿真 LVDS 绑 ODDR | ✅ 实测到 bit |
| ② pclk 供给 | ✅ 方案 F(BUFIO), UG301 §4.1 背书 |
| ③a LUT | ✅ 优化后 26–29% |
| ④ 电压/引脚 | ✅ **本次核掉, 非障碍** |
| ③b 200MHz 时序 | ⚠ 余 2.1% → **降 700M(175MHz)后 16.7%** |
| ⑤ IOL 内相位 | 🔴 工具不查, 只能上板量 |
| — 驱屏共模 | 🔴 规格无保证 |

⇒ **EG4S20CG324 在 700 Mbps 下只剩两条隐患, 且都是小验证板一次能量完的。**
