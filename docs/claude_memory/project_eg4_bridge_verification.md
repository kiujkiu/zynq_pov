...
## ✅🔄 ③a-2 LUT 优化实测 (2026-09-03 晚, `td_eg4_port/rtl/opt_*`, 提交 787cd8e / e61277f / 4fba725 / be5de92) —— **翻盘**

三个模块各自独立优化并在 EG4S20CG324 上实测(功能等价, 均有对拍仿真证据):

| 模块 | 改前(gate) | **改后(gate)** | 降幅 | 手法 |
|---|---|---|---|---|
| u_ack | 1,143 | **72** | −94% | 消 17 个变址读口(TD 每口复制整份 DRAM) ⇒ 改纯串行解析(尾部移位寄存器 = CHKSUM, CRC 延后 16 位在线算) |
| u_seq | 402 | **133**(桥片工作参数) | −67% | 寄存器写序列表进 1 块 bram9k(原是 TD 把 `$readmemh` 常量折叠成 LUT-ROM) |
| u_tx | 426(N=1) | **125 + 54×N** | N=1 时 −58% | 拆共享控制器(独热状态机, 不随链数变) + 逐 lane 数据通路; N=1/2/4 三点验证线性 |

**桥片总量估算(24 链, 三个独立实测相加, 不是一次合成)**:
```
u_ack×1(72) + u_seq×1(133) + u_tx×24(125+54×24=1421) + 上行RX(2290,已单独综合) + 顶层胶合(~75估) ≈ 4,000
占 EG4S20 19,600 LUT4 ≈ 20.4%(若按观察到的 gate→phy 最坏膨胀 +37% 折算上限 ≈ 24%)
```
详见 `td_eg4_port/OPT_SUMMARY.md`(含口径警告: gate/phy 混用、N=24 是外推、胶合逻辑未建)。

**⇒ LUT 这条轴从"明确不够(141–194%)"翻盘成"明确够用(20–24%)"。**
但**不改变整体排序** —— ③b(200MHz 2% 余量, 4:1 gearing 逼出来的)、④(Bank1/3 锁 3.3V)、
⑤(IOL 相位只能上板量) 三条**不受 LUT 优化影响**, EG4S20 仍排在 `reference_bridge_fpga_alternatives.md`
推荐的 **GW5A-LV60UG324S**(8:1~16:1 gearing, 无 Bank 锁死)和 **PH1A60GEG324** 之后。

🎯 **但这份优化工作不是白做的**: 三种手法(变址读→串行化 / 常量表→BRAM / 共享控制+逐 lane)通用,
换到最终选定的任何芯片上都适用，只是 GW5A-60/PH1A60 富余量大，不优化也过得去。

## 🔴 ③a-1 LUT 真数首测(未优化) —— **EG4S20 装不下** (`td_eg4_port/`, 提交 0b184a1, 已被上面的优化结果部分推翻)
