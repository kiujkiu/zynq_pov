...
## 🔬 2026-09-23 上机第一步: 冒烟测试(不是完整桥), 板级钉死引脚撤回

用户手里有 ACG525 教学板 + 下载器, 但**没接任何外部器件**(DR1M90/ICND2260 都没接)。
本想把 `clk_ref_ul/dp`/`rstn_pin`/`led[0..2]` 钉死到 ACG525 板载真实资源
(小梅哥引脚信息表.xlsx 核过: CLK_G(50M)=T9, KEY0=B16, LED0/1/2=D14/C14/B9,
与 [[project_gw5a_bridge_verification]] 里 `gw5a_loop` 已验证过的用法一致),
**但撞上新的硬约束**: 这几个球分散在 bank 0/1/4 三个不同 bank, 而 GW5A 的
**VCCIO 是整个 bank 共享, 不是按球分**(与上文"bank 两两短接"是另一条独立规律)
——一旦某 bank 里有 1 个 3.3V 单端信号, 整个 bank 就不能再放 2.5V LVDS25。
干净的 bank 只剩 {2,3,6,7}=56 位点, 不够桥片要的 86 対。

⇒ **判断**: 86 対现在没接任何东西, 硬凑真实引脚意义不大。改分两条线:
1. `tools/gen_pins.py` 的板级钉死方案**撤回**, 桥片主设计维持"BIT:OK+时序收敛"
   不变, 仍用工具自选的合法位点(不对应任何真实板级布线)。
2. 新增独立的 `smoke/` 最小验证项目 (`gw5a_smoke_top.v` + `build_smoke.sh`):
   只用 5 个真实板载引脚(clk/rstn/led), 目的只是验证"设计→gw_sh→JTAG下载器→
   真实 GW5A-25A 芯片"这条链路通不通(PLL 锁不锁、LED 亮不亮), 完全不碰 86 対。
   已出 `smoke_verA.fs`/`smoke_verB.fs`(芯片版本 A/B 未定先都出), 待用户用
   Gowin Programmer 实际下载确认: LED0 常亮(锁定)/LED1 ~1.1Hz 慢闪(dclk 真跑)/
   LED2 跟 KEY0 走(输入脚通)。

📌 **要真正验证 86 対桥片(与 DR1M90/ICND2260 对接), 得先知道 ACG525 教学板
到底有没有把这些差分脚引到可焊接/可跳线的排针上** —— 这与冒烟测试用的
"labeled 外设(LED/KEY/CLK)"是完全不同的问题, 需要另外查这块教学板的完整排针
表(不只是标注了功能的那些脚), 是下一次真要接线时才要做的事。
