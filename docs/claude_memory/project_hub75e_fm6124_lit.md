...
## 实测性能 (2026-06-02 frame_count delta @ TUNIT=32)

| 模式 | CTRL | fps | 注 |
|---|---|---|---|
| SERIAL | 0x521 (use_fb=1) | **462** | 跟理论 460 fps 对上 |
| OVERLAP | 0x561 (use_fb=1) | **645** | +39%, datasheet 声称 "+50%" |

视觉对比: 两 mode grid 0-31 数字 + R→G→B 渐变完全一致 (mean 186 vs 185), BCM 权重保留.
...
按 ROI 排:
1. **DCLK 30 MHz spec 上限** (FM6124 max): 当前 18.75 MHz, 升到 30 MHz → 帧率 ~1.6×
   - 选项 A: DCLK_DIV=2 不动 aclk → 37.5 MHz (超 spec 25%, 边缘可能不稳)
   - 选项 B: MMCM aclk 120 MHz + DCLK_DIV=4 → 精确 30 MHz, spec 内
   - 选项 C: MMCM aclk 60 MHz + DCLK_DIV=2 → 精确 30 MHz, spec 内
2. **8-bit BCM**: BCM_PLANES 6→8, 色深 64→256 灰阶, fps 降到 ~287 (serial) / ~400 (overlap)
3. **HDMI 桥/POV-3D 数据源**: 把 HDMI VDMA → fb 或 HLS slice → fb, 不再静态 fb
4. **多 panel 拼接**: 物理 chain 第二块, PANEL_WIDTH 改
