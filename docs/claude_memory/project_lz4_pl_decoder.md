...
description: 字节串行 + 64KB 片上历史; 0.93-0.96 B/clk 离线估被 08-25 上板 0.95 精确印证; **2026-08-25 已在 Zynq 上跑通三引擎 74ms/帧, 828/828 帧**; '卡在安路下载器'那条阻塞是伪命题
type: project
---
...
🔴 **第 4 条"卡在下载器"这个前提本身是错的, 两头都不成立**:
- **Zynq 侧根本不需要安路下载器** —— 本文 §"Zynq 可以当这个核的首次上板平台"预判对了,
  08-25 走的正是这条路。
- **DR1 侧也不需要** —— DR1 走 Linux `fpga_manager` + configfs overlay 加载 bitstream,
  秒级重配, **不用 JTAG 不用重启**([[feedback_dr1_load_bit_without_jtag]])。
  下载器只在 TD 的在线调试/烧 flash 时才需要。
⚠ 这条"阻塞"在记忆里挂了近三周(08-06 ~ 08-25), 期间它一直是 DR1 线的头号借口。
**教训: 写"卡在 X"之前先确认 X 是不是真的在关键路径上。**

# 🎯 2026-08-25 Zynq 上板跑通 (commit `7efe755` / `970edce`)

**三个引擎挂 HP3/HP1/HP2, AXI-Lite 在 `0x40020000/30000/40000`。**

| | 实测 |
|---|---|
| 解码 | **74 ms/帧**(10.47 MB), 即 **0.95 B/clk** —— 与本文 0.93-0.96 的离线数**完全吻合** |
| 可靠性 | **828/828 帧全走 3str/PL**, fb=0, **超时 0 次** |
| 改造前 | `dec 158ms + cpy 80ms = 238ms/帧` ⇒ 5.5 fps |
| 改造后 | 5.5 → **10-12 fps**(再叠 fold_a 把解码压到 55.5 ms) |
| `pair_miss` | 三引擎全速打 DDR 后**仍是 0.0/s** ⇒ [[feedback_pov_4x_ip_breaks_hdmi]] 没有重演 |
| `CYCLES` | 实测 3646284/3698333/3691471, 全部落在事前预测的 3.63-3.77M 区间内 |

## 🔴 集成时挖出来的一个核外 bug: `lz4_axi_top` 的 AXI-Lite 会挂死

`awready` 恒 1 且写只在 `awvalid && wvalid` **同拍**生效
⇒ **AW 先到就被吃掉且永不回 B** ⇒ 挂死 GP0(面板寄存器跟着全挂)。
**原 tb 总是同拍拉两个 valid, 所以从没暴露。**
修复在新写的适配壳 `lz4_engine_axi.v`(498 行)里 —— 本文那两个核文件
(`lz4_decode_core.v` / `lz4_axi_top.v`)**一行没改**, 2026-08-26 `diff -q` 确认与
`dr1v90/lz4hw/rtl/` 下的原件**逐字节相同**([[reference_zynq_dr1_code_drift]])。

🔴 **这个 bug 在 DR1 上会更痛**: DR1 GP0 挂死 = 整板失联, 而 Zynq 还能靠 JTAG 救。
同类风险见 [[feedback_dr1_pl_probe_must_be_hang_proof]]。

## 两条集成期的实测结论

- **16 拍突发打包器**: `BURST_LEN=16` 原本是死参数(实际 `m_awlen=0` ⇒ 单拍 8B)。
  修好后写事务 **14.53M/s → 0.91M/s**, PL 侧 DDR 占用 **348 → 231 MB/s**, **吞吐一分没掉**。
- **`axi_protocol_converter` 替掉 SmartConnect**(1:1 直通不需要交叉开关):
  **LUT 12432→8010**, 时钟网 fanout 14423→8726, **WHS 0.010→0.020**。
  ⚠ 但 4 引擎版 fanout 8726→**17092** 而 WHS **纹丝不动**
  ⇒ "fanout↑ ⇒ 偏斜↑ ⇒ WHS↓"**至少不完整, 别当定律用**。

## 引擎数: 现役 3 个 + 备用 4 个

`create_panel_proj_v6.tcl` 现在是 `set NENG 4` / `HP_ORDER {3 1 2 3}`(lz4_0 与 lz4_3 共享 HP3),
但**现役上板的是 3 引擎版**(`pl_lz4x3.bit.bin`) —— fold_a 已把解码压到
55.5 ms < 62 ms 圈周期, 瓶颈换成翻页相位, **再加引擎 flip 不会变高**。
4 引擎版 WNS 6.633 / WHS 0.020 / LUT 12053(22.66%) / RAMB36 82(**58.57%**), `NENG` 改回 3 就逐字回到现役版。

## ⇒ 对 DR1 的意义变了

本文原来把"上板"当终点。现在 **DR1 侧不再是"从零做", 而是"换一套 HP 拓扑接进去"** ——
核和壳都是现成且验过的, 新工作全在 **DR1 只有 2 个 HP** 这件事上,
见 [[project_dr1_hp_port_shortage]] / [[project_dr1_parity_plan]] B 线。
