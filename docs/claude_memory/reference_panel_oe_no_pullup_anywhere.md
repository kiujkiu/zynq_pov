...
- 高阻期要靠片内弱上拉拉过屏的 VIH≈2.66V，而 **DS1200 表 3-8 的 IPU 全是 "TBD"** ⇒ 纸面无法论证
  🔴 **2026-08-27: 这个 "TBD" 的前提要重查** —— 我们手上那份 DS1200 是 **0.4 草稿
  (全文 215 个 TBD)**，而 **V1.0 正式版 0 个 TBD 且表号整个重排过**
  （[[reference_dr1_datasheet_draft_vs_v10]]）。同一份草稿今天已经在 LVDS 速率和 LVDS 直流
  参数上骗过两次。⚠ **本条尚未在 V1.0 里核对过**，所以不是"IPU 有值了"，而是
  **"'厂商没测'这个判断没有依据，先去 V1.0 里找 3-8 的对应表"**。
