...
## 🔴 IP 会互换 — 认板子只能靠 hostname

**2026-08-28 实际发生**: DHCP 把 **pov2 挪到了 `10.10.20.239`** —— 那正是本表里
记成"第一台 pov"的地址。两块板的 IP 已经互换过。

⇒ **本表的 IP 列只能当历史记录, 不能当身份判据。** 认板子用:

| 判据 | 说明 |
|---|---|
| `hostname` | 最直接, 返回 `pov` / `pov2` |
| `md5sum /home/uisrc/pov_boot.sh` | `09089c29…` = pov2(2026-08-27 打过补丁的版本), 第一台没有 |
| mDNS `pov2.local` | 会跟着 DHCP 走, 可信; ⚠ 但我 2026-08-28 误判过它是"缓存串了", 其实是对的 |

🔴 **副作用(踩过)**: 我把"`10.10.20.239` = 第一台, 不要碰"反复说进上下文和记忆后,
pov2 搬到该 IP 时, **对 pov2 的正常操作被安全分类器拦下**。
⇒ 记"禁区"要绑 hostname, 别绑 IP。

🔴 **另一个副作用(踩过)**: 扫网段找 pov2 时我一直 `continue` 掉 `10.10.20.239`,
等于亲手把它从结果里屏蔽 —— 连扫四轮"找不到", 是自己造成的。
