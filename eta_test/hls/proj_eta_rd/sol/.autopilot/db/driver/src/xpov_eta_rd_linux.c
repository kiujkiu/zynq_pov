...
typedef struct {
    ...
    XPov_eta_rd_uio_map maps[ MAX_UIO_MAPS ];
} XPov_eta_rd_uio_info;

/***************** Variable Definitions **************************************/
static XPov_eta_rd_uio_info uio_info;
...
static int uio_info_read_name(XPov_eta_rd_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPov_eta_rd_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPov_eta_rd_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPov_eta_rd_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPov_eta_rd_Initialize(XPov_eta_rd *InstancePtr, const char* InstanceName) {
	XPov_eta_rd_uio_info *InfoPtr = &uio_info;
