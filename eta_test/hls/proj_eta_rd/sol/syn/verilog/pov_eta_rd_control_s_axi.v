...
// ==============================================================
`timescale 1ns/1ps
module pov_eta_rd_control_s_axi
#(parameter
    ...
    output wire [31:0]                   pf_cols,
    input  wire [63:0]                   xor_out,
    input  wire                          xor_out_ap_vld
);
//------------------------Address Info-------------------
...
localparam
    ...
    ADDR_PHASE_DATA_0       = 7'h1c,
    ADDR_PHASE_CTRL         = 7'h20,
    ADDR_N_SLOTS_DATA_0     = 7'h24,
    ADDR_N_SLOTS_CTRL       = 7'h28,
    ADDR_AXIS_OFF_Q8_DATA_0 = 7'h2c,
    ADDR_AXIS_OFF_Q8_CTRL   = 7'h30,
    ADDR_MIRROR_U_DATA_0    = 7'h34,
    ADDR_MIRROR_U_CTRL      = 7'h38,
    ADDR_PF_COLS_DATA_0     = 7'h3c,
    ADDR_PF_COLS_CTRL       = 7'h40,
    ADDR_XOR_OUT_DATA_0     = 7'h44,
    ADDR_XOR_OUT_DATA_1     = 7'h48,
    ADDR_XOR_OUT_CTRL       = 7'h4c,
    WRIDLE                  = 2'd0,
    WRDATA                  = 2'd1,
    WRRESP                  = 2'd2,
    WRRESET                 = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_pf_cols = 'b0;
    reg                           int_xor_out_ap_vld;
    reg  [63:0]                   int_xor_out = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_PF_COLS_DATA_0: begin
                    rdata <= int_pf_cols[31:0];
                end
                ADDR_XOR_OUT_DATA_0: begin
                    rdata <= int_xor_out[31:0];
                end
                ADDR_XOR_OUT_DATA_1: begin
                    rdata <= int_xor_out[63:32];
                end
                ADDR_XOR_OUT_CTRL: begin
                    rdata[0] <= int_xor_out_ap_vld;
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_PF_COLS_DATA_0)
            int_pf_cols[31:0] <= (WDATA[31:0] & wmask) | (int_pf_cols[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_xor_out_ap_vld <= 1'b0;
    else if (ACLK_EN) begin
        if (xor_out_ap_vld)
            int_xor_out_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_XOR_OUT_CTRL)
            int_xor_out_ap_vld <= 1'b0; // clear on read
    end
end
