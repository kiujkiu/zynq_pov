...
entity pov_eta_wr_control_s_axi is
generic (
...
end entity pov_eta_wr_control_s_axi;
...
architecture behave of pov_eta_wr_control_s_axi is
    ...
    constant ADDR_SLOT_BASE_DATA_0   : INTEGER := 16#1c#;
    constant ADDR_SLOT_BASE_DATA_1   : INTEGER := 16#20#;
    constant ADDR_SLOT_BASE_CTRL     : INTEGER := 16#24#;
    constant ADDR_PHASE_DATA_0       : INTEGER := 16#28#;
    constant ADDR_PHASE_CTRL         : INTEGER := 16#2c#;
    constant ADDR_SLOT_START_DATA_0  : INTEGER := 16#30#;
    constant ADDR_SLOT_START_CTRL    : INTEGER := 16#34#;
    constant ADDR_N_SLOTS_DATA_0     : INTEGER := 16#38#;
    constant ADDR_N_SLOTS_CTRL       : INTEGER := 16#3c#;
    constant ADDR_AXIS_OFF_Q8_DATA_0 : INTEGER := 16#40#;
    constant ADDR_AXIS_OFF_Q8_CTRL   : INTEGER := 16#44#;
    constant ADDR_MIRROR_U_DATA_0    : INTEGER := 16#48#;
    constant ADDR_MIRROR_U_CTRL      : INTEGER := 16#4c#;
    ...
    signal int_slot_base       : UNSIGNED(63 downto 0) := (others => '0');
    signal int_phase           : UNSIGNED(31 downto 0) := (others => '0');
    signal int_slot_start      : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_SLOT_BASE_DATA_0 =>
                        rdata_data <= RESIZE(int_slot_base(31 downto 0), 32);
                    when ADDR_SLOT_BASE_DATA_1 =>
                        rdata_data <= RESIZE(int_slot_base(63 downto 32), 32);
                    when ADDR_PHASE_DATA_0 =>
                        rdata_data <= RESIZE(int_phase(31 downto 0), 32);
                    when ADDR_SLOT_START_DATA_0 =>
                        rdata_data <= RESIZE(int_slot_start(31 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    slot_base            <= STD_LOGIC_VECTOR(int_slot_base);
    phase                <= STD_LOGIC_VECTOR(int_phase);
    slot_start           <= STD_LOGIC_VECTOR(int_slot_start);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_slot_base(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SLOT_BASE_DATA_0) then
                    int_slot_base(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_slot_base(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_slot_base(63 downto 32) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SLOT_BASE_DATA_1) then
                    int_slot_base(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_slot_base(63 downto 32));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_slot_start(31 downto 0) <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SLOT_START_DATA_0) then
                    int_slot_start(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_slot_start(31 downto 0));
                end if;
            end if;
        end if;
    end process;
